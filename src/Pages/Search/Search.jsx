...
export default class Search extends Component {
    ...
    search = (page, searchKey) => {
        ...
        if (this.props.location.pathname) {
            ...
            localStorage.setItem('searchQuery', this.props.location.pathname.substr(8))
        }
        else
            searchKey = localStorage.getItem('searchQuery');


        axios.get(`https://api.themoviedb.org/3/search/multi?api_key=0c46ad1eb5954840ed97f5e537764be8&page=${page}&query=${searchKey}`)
            .then(res => {
    ...
    handleObserver(entities) {
        ...
        if (this.state.prevY > y) {
            ...
            this.setState({ page: curPage, searchKey: this.props.location.pathname.substr(8) });
        ...
        return <>
            ...
            <section className="container tv" style={{ minHeight: "75.5vh" }}>
                <div className="row" >
                            ...
                            React.Children.toArray(  // that handle a unique key itself
                                this.state.movies.map((value) => {
                                    return (
                                        value.poster_path ?
                                            (
                                                <div className="item col-xl-3 col-lg-4 col-md-6 col-sm-6 my-2 card card-body" onClick={() => {
                                                    if (value.media_type === "movie")
                                                        return this.goToMovieAbout(value);
                                                    else if (value.media_type === "tv")
                                                        return this.goToTvAbout(value);
                                                    else
                                                        return this.goToPersonAbout(value);
                                                }
                                                }>
                                                    <div className="text-center position-relative mb-2">

                                                        <div className="captionLayer overflow-hidden mb-2">
                                                            <img src={`https://image.tmdb.org/t/p/original/${value.poster_path}`} className="w-100 h-100" alt={value.title !== "undefined" ? value.title : value.name} title={value.title === "undefined" ? value.name : value.title} />
                    ...
                    <div ref={loadingRef => (this.loadingRef = loadingRef)} style={{ height: "100px", margin: "30px auto" }} >
                        <span style={{ display: this.loading ? "block" : "none" }} className="py-2 text-center">
                            <Loader type="Puff" color="#00BFFF" height={80} width={80} time={3000} />
                        </span>
                    </div>
