...
const MovieRecommendations = ({ movie, goToMovieAbout }) => {
    ...
    const fetchImages = async () => {
        await axios.get(`https://api.themoviedb.org/3/movie/${movie.id}/recommendations?api_key=0c46ad1eb5954840ed97f5e537764be8`)
            .then((response) => {
    ...
    return loading ? (
        <>
            ...
            <div className="item text-center my-3">
                <h3>Recommendations Movies as <b className="text-info">{movie.title ? movie.title : movie.name}</b> to Watch Now</h3>
            </div>
