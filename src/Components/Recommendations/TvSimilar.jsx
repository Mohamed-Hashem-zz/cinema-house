...
const TvSimilar = ({ series, goToTvAbout }) => {
                    ...
                    <div className="item card card-body" onClick={() => goToTvAbout(poster)}>
                        <div className="text-center position-relative">
                            <div className="captionLayer overflow-hidden mb-2  carouselItem">
                                <img src={`https://image.tmdb.org/t/p/original${poster.poster_path}`} width="100%" height="350px" className="carouselItem__img" alt={poster.title === "undefined" ? poster.name : poster.title} title={poster.title === "undefined" ? poster.name : poster.title} onDragStart={handleDragStart} />
    ...
    const fetchImages = async () => {
        await axios.get(`https://api.themoviedb.org/3/tv/${series.id}/similar?api_key=0c46ad1eb5954840ed97f5e537764be8`)
            .then((response) => {
    ...
    return loading ? (
        <>
            ...
            <div className="item text-center my-3">
                <h3>Similar Series as <b className="text-info">{series.title ? series.title : series.name}</b></h3>
            </div>
